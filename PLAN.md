...
- 解析 Bond 成員、VLAN 子介面、VIP／Cluster IP、Heartbeat／HB 介面與介面註解。
...
- `cphaprob -a if` 的介面行出現 `(S)` 時判定為 Heartbeat；若沒有該輸出，才以介面註解作備援判定。
...
2. 移除「成員 IP 減一」推測 Cluster IP 的邏輯，只顯示設定檔明確宣告的 Cluster IP。
3. 補強 HA Cluster 判定，避免僅憑 Cluster mode、同 `/24` 與相鄰管理 IP 誤合併設備。
4. 以 `Checkpoint_CONFIG` 及人工案例建立 parser 回歸測試：單機、HA、VLAN、Bond、無管理介面、SMS 直連／靜態路由與 `cphaprob -a if`。
5. 補強 parser 對不同 Gaia 版本、路由屬性、註解格式及多種 Bond／VLAN 寫法的容錯。
6. 將解析、資料模型、SVG 繪圖與互動控制拆成模組，降低單一 `index.html` 的維護成本。

## 開發原則

- 所有設定檔只在瀏覽器本機解析，不上傳伺服器。
- 以 IPv4 嚴格驗證與最長前綴匹配作為路由判斷基礎。
- Heartbeat 以 `cphaprob -a if` 的 `(S)` 標記為優先依據；註解只可作為缺少該輸出時的備援。
